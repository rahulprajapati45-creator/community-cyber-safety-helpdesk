require("dotenv").config();

const express = require("express");
const axios = require("axios");
const router = express.Router();

const FraudNumber = require("../models/FraudNumber");

// Check mobile number
router.post("/check-number", async (req, res) => {

    try {

        const { phone } = req.body;

        if (!phone) {
            return res.json({
                success: false,
                message: "Please enter a mobile number"
            });
        }

        const number = phone.trim();

        // Check 10 digit Indian mobile number
        if (!/^[6-9][0-9]{9}$/.test(number)) {
            return res.json({
                success: false,
                message: "Please enter a valid 10-digit mobile number"
            });
        }

        const result = await FraudNumber.findOne({
            phone: number
        });

        if (!result) {

    try {

        const response = await axios.get("https://api.veriphone.io/v2/verify", {
            params: {
                phone: number,
                key: process.env.VERIPHONE_API_KEY
            }
        });

        const data = response.data;

        if (data.phone_valid === true) {
           return res.json({
    success: true,
    status: "valid",
    message: "Number is valid. No fraud record found.",
    phoneType: data.phone_type,
    country: data.country,
    carrier: data.carrier
});
        }

        return res.json({
            success: true,
            status: "unknown",
            message: "⚠️ Number could not be verified."
        });

    } catch (apiError) {

        console.error("Veriphone API Error:", apiError.message);

        return res.json({
            success: true,
            status: "unknown",
            message: "Number is not verified in our database."
        });
    }
}

        if (result.status === "fraud") {
            return res.json({
                success: true,
                status: "fraud",
                message: "⚠️ This number is reported as FRAUD / SPAM."
            });
        }

        if (result.status === "safe") {
            return res.json({
                success: true,
                status: "safe",
                message: "✅ This number is marked as SAFE."
            });
        }

    } catch (error) {

        console.error(error);

        res.status(500).json({
            success: false,
            message: "Server error"
        });
    }
});

module.exports = router;